...
} from "@/components/share/ProfileShareCard";
...
export async function createProfileShareBlob(
  ...
  try {
    root.render(<ProfileShareCard payload={payload} isDark={isDark} />);
    await waitForShareCardAssets(host);

    const el = host.querySelector("[data-share-card-root]") as HTMLElement | null;
    if (!el) throw new Error("ProfileShareCard introuvable");
...
export async function shareProfilePng(
  ...
  const blob = await createProfileShareBlob(payload, isDark);
  const file = new File([blob], "one-more-profil.png", { type: "image/png" });
  const text = `${UI.profileShareText} · ${payload.displayName}`;
  ...
  a.href = url;
  a.download = `one-more-profil-${Date.now()}.png`;
  a.click();
